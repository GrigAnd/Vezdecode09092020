  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link /*hidden={price == 0}*/ to={(price == 0) ? `/place/${area.id}/${item.id}` : `/basket/${area.id}/${item.id}`} /*className="Place__order"*/className={(price == 0) ? "Place__order_inactive" : "Place__order"}>
        {(price == 0) ? `Выберите товар` : `Оформить заказ (${price})`}
        </Link>
      </footer>
    </div>
  );
